...
namespace NewBookRentalShopApp
{
    public partial class FrmBookInfo : MetroForm
    {
        ...
        private void InitInputData()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
                {
                    ...
                    // SqlDataReader = 개발자가 하나씩 처리할 때 쓰는 것
                    // SqlDataAdapter, DataSet = 한번에 데이터 그리드뷰 등에 뿌릴 때
                    ...
                    while (reader.Read())
                    {
                        // key, Value
                        // B001, 공포/스릴러
                        // reader[0] = Division 컬럼, reader[1] = Names 컬림
        ...
        {
            isNew = true;
            TxtBookIdx.Text = TxtBookIdx.Text = TxtAuthor.Text = string.Empty;
            TxtBookIdx.Focus(); // 입력창 맨 처음 컨트롤에 포커스
            TxtBookIdx.Text = TxtBookIdx.Text = TxtAuthor.Text = string.Empty;  // 입력, 수정, 삭제 이후에는 모든 입력값을 지워줘야 함
            CboDivision.SelectedIndex = -1;
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
                {
                    ...
                    SqlParameter prmRealeasDate = new SqlParameter("@ReleaseDate", DtpReleaseDate.Value);
        ...
        {
            if (string.IsNullOrEmpty(TxtBookIdx.Text)) // 사용자아이디순번이 없으면
            {
        ...
        private void RefreshData()
        {
            using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
            {
                ...
                var query = @"SELECT b.[bookIdx]
                                   , b.[Author]
                                   , b.[Division]
                                   , d.Names AS DivNames   -- 책장르에 구분명
                                   , b.[Names]          -- 책 제목
                                   , b.[ReleaseDate]
                                   , b.[ISBN]
                                   , b.[Price]                                      FROM [bookstbl] AS b
                                JOIN divtbl AS d
        ...
        private void DgvResult_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1) // 아무것도 선택하지 않으면 -1
            {
                ...
                // 콤보박스는 맨 마지막에
                //MessageBox.Show(selData.Cells[3].ToString());
