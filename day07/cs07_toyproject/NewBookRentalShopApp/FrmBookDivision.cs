...
namespace NewBookRentalShopApp
{
    public partial class FrmBookDivision : MetroForm
    {
        ...
        {
            isNew = true;
            TxtDivision.Text = TxtDivision.Text = TxtNames.Text = string.Empty;
            TxtDivision.ReadOnly = false;   // 최초 입력할 때는 PK값을 입력해줘야 함
            TxtDivision.Focus(); // 입력창 맨 처음 컨트롤에 포커스
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
                {
                    ...
                    SqlParameter prmNames = new SqlParameter("@Names", TxtNames.Text);  // 암호화 끝.
            ...
            TxtDivision.Text = TxtDivision.Text = TxtNames.Text = string.Empty;  // 입력, 수정, 삭제 이후에는 모든 입력값을 지워줘야 함
            RefreshData();
        }
        ...
        private void DgvResult_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1) // 아무것도 선택하지 않으면 -1
            {
                ...
                TxtDivision.ReadOnly = true;  // UPDATE시는 PK인 Division을 변경 해야함
